...
@Service
public class BalanceToolkitManager {
	...
	public BalanceToolkitManager(){}
	
	public BalanceToolkitManager(String appName){
		this.appName = appName;
	}
			
	public int auditBalanceDiscrepancy(){
	...
	public void auditAndSaveBalanceDiscrepancy(String appName){
		System.out.println("Start Audit Balance Discrepany Between Kenan and Dynamic Balances");
		long startTime = System.currentTimeMillis();
		BalanceAudit balanceAudit = null;
		List<BalanceAudit> balanceAuditList = new ArrayList<BalanceAudit>();
		Set<DeviceInfo> deviceSet = overallAuditService.getActiveAndSusppendedDevices();								
		for(DeviceInfo di : deviceSet){
			balanceAudit = overallAuditService.auditAndSaveBalanceDiscrepancy(di, appName);
			if(balanceAudit != null) {				
				balanceAuditList.add(balanceAudit);	
			}   
		}
		if(balanceAuditList != null && balanceAuditList.size() > 0)
			balanceAuditService.batchSaveBalanceAudit(balanceAuditList);
		
		long endTime = System.currentTimeMillis();
		long totalTime = endTime - startTime;
		System.out.printf("%d Accounts Found with Balance Discrepany %n", balanceAuditList.size());
		System.out.printf("Total Time = %d Sec. %n", totalTime/1000);
	}
	...
	public BalanceAudit getBalanceAudit(int accountNo){
		return balanceAuditService.getBalanceAudit(accountNo);
	}
	
	public Set<BalanceAudit> getAllBalanceAuditRecords(){
		return balanceAuditService.getAllBalanceAuditRecords();
	}
	
	public void saveBalanceAudit(BalanceAudit balanceAudit){
	    balanceAuditService.saveBalanceAudit(balanceAudit);
	}
